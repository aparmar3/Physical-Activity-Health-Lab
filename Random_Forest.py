...
hip_labels = np.array(hip['Joints'])
# Remove all columns that will not be used as features. Only features are mean, sd, angle
# axis 1 refers to the columns
hip_features = hip.drop(['RTExercise', 'Exercise Type', 'Joints'], axis=1)
...
hip_rf = RandomForestClassifier(n_estimators = 3000, warm_start = True, max_features='auto', criterion='entropy', n_jobs=-1)
...
# break the file up into the features and labels
# Labels are the values we want to predict
wrist_labels = np.array(wrist['Joints'])
# Remove all columns that will not be used as features. Only features are mean, sd, angle
# axis 1 refers to the columns
wrist_features = wrist.drop(['RTExercise', 'Exercise Type', 'Joints'], axis=1)
# Saving feature names for later use
...
# Import the model we are using
# Instantiate model with 1000 decision trees
wrist_rf = RandomForestClassifier(n_estimators = 3000, warm_start = True, max_features='auto', criterion='entropy', n_jobs=-1)
print("done instantiating model")
# Train the model on training data
...
# break the file up into the features and labels
# Labels are the values we want to predict
hip_and_wrist_labels = np.array(hip_and_wrist['Joints'])
# Remove all columns that will not be used as features. Only features are mean, sd, angle
# axis 1 refers to the columns
hip_and_wrist_features = hip_and_wrist.drop(['RTExercise', 'Exercise Type', 'Joints'], axis=1)
# Saving feature names for later use
...
hip_and_wrist_rf = RandomForestClassifier(n_estimators = 3000, warm_start = True, max_features='auto', criterion='entropy', n_jobs=-1)
print("done instantiating model")
# parameters = {
...
print("The feature importances were:")
print(hip_and_wrist_rf.feature_importances_)
print("Here are the predictions")
# print(hip_and_wrist_predictions)
# (row,) = hip_and_wrist_predictions.shape
# (row1,) = hip_and_wrist_test_labels.shape
# print(row)
# print(row1)
# df = pd.DataFrame(np.array(hip_and_wrist_predictions).reshape(row, 1))
# df.to_csv('Predictions2.csv')
# df1 = pd.DataFrame(np.array(hip_and_wrist_test_labels).reshape(row1, 1))
# df1.to_csv('Actual2.csv')
